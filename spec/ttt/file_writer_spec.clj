...
(describe "write-players"
  (it "adds current player marker and role to respective key"
    (should= "\"current-player\": {\"marker\":{\"symbol\":\"x\",\"color\":\"green\"},\"role\":\"human\"}"
             (file-writer/write-players "current-player" { :marker {:symbol "x" :color "green"}
                                                           :role :human })))

  (it "adds opponent marker and role to respective key"
    (should= "\"opponent\": {\"marker\":{\"symbol\":\"o\",\"color\":\"green\"},\"role\":\"hard-computer\"}"
             (file-writer/write-players "opponent" { :marker {:symbol "o" :color "green"}
                                                     :role :hard-computer }))))

(describe "write-json"
  (it "returns a map in valid JSON format"
    (should= (str "{\"board\": [\"_\",\"_\",\"_\",\"_\",\"_\",\"_\",\"_\",\"_\",\"_\"],"
                  "\"current-player\": {\"marker\":{\"symbol\":\"x\",\"color\":\"green\"},\"role\":\"human\"},"
                  "\"opponent\": {\"marker\":{\"symbol\":\"o\",\"color\":\"green\"},\"role\":\"easy-computer\"}}")
